#!/bin/bash -l
#SBATCH --output=/scratch/prj/stem_cells_pituitary/Georgia/scripts/0_outputs/2_ChromBPnet_exp22.out
#SBATCH --job-name=ChromBPnet_tat1
#SBATCH --gres=gpu
...
# Specific Paths
experimentDir="/scratch/prj/stem_cells_pituitary/Bence/atacseq_results/22"
experiment_name="experiment_22"
Tn5_bias_model="/scratch/prj/stem_cells_pituitary/Georgia/ChromBPnet/Bias_Models/Tn5_bias_model_experiment_22_bias0.9"
...
chrombpnet pipeline \
	-ibam "$experimentDir/bwa/merged_replicate/TaT1.mRp.clN.sorted.bam" \
        -d "ATAC" \
        -g "$genome_fasta" \
        -c "$genome_chrom_subset" \
        -p "$experimentDir/bwa/merged_replicate/macs2/broad_peak/TaT1_peaks_no_blacklist_2.bed" \
        -n "$experimentDir/bwa/merged_replicate/macs2/broad_peak/TaT1_output_negatives.bed" \
        -fl "$inputDir/ChromBPnet/splits/$genome/fold_0.json" \
        -b "$Tn5_bias_model/models/bias.h5" \
        -o "$modelDir/${experiment_name}_model_TaT1"
