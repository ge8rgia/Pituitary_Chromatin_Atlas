#!/bin/bash -l
#SBATCH --output=/scratch/prj/stem_cells_pituitary/Georgia/scripts/0_outputs/1_Tn5bias_exp.out
#SBATCH --job-name=Tn5b_exp10
#SBATCH --gres=gpu
...
peaksDir="/scratch/prj/stem_cells_pituitary/Bence/atacseq_results/$exp/bwa/merged_replicate/macs2/broad_peak/consensus"
...
chrombpnet bias pipeline \
        -ibam "$experimentDir/AtT20_NEO.mRp.clN.sorted.bam" \
        -d "ATAC" \
        -g "$genomeDir/${genome}.fa" \
        -c "$genomeDir/${genome}.chrom.sizes" \
        -p "$peaksDir/att20_peaks_no_blacklist_2.bed" \
        -n "$peaksDir/att20_output_negatives.bed" \
