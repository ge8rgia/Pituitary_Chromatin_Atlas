...
do
	...
	bedtools genomecov -i $OUT/${cell_type}/${cell_type}_temp.bed -bg -g genome/mm10.chrom.sizes > $OUT/${cell_type}/${cell_type}_observed.bdg

	echo "Convert bedgraph to bigwig for ${cell_type}"
	bedGraphToBigWig $OUT/${cell_type}/${cell_type}_observed.bdg genome/mm10.chrom.sizes $OUT/${cell_type}/${cell_type}_observed.bw
