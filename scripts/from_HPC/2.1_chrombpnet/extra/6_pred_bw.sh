#!/bin/bash -l
#SBATCH --output=/scratch/prj/stem_cells_pituitary/Georgia/scripts/0_outputs/6_pred_bw.out
#SBATCH --job-name=pred_bw
#SBATCH --gres=gpu
#SBATCH --exclude=erc-hpc-comp038,erc-hpc-comp039,erc-hpc-comp034
...
echo "Begin pred_bw: $(date)"

# Run predicted bigwig files command
chrombpnet pred_bw \
	-bm ChromBPnet/Bias_Models/Tn5_bias_model_f0/models/bias.h5 \
	-cm ChromBPnet/Models/Lactotrophs_model/models/chrombpnet.h5 \
	-cmb ChromBPnet/Models/Lactotrophs_model/models/chrombpnet_nobias.h5 \
	-r ChromBPnet/data/consensus_chromatin_landscape_modified.bed.gz \
	-g genome/mm10.fa \
	-c genome/mm10.chrom.sizes \
	-op ChromBPnet/outputs/Lactotrophs/pred_bw/predicted_lactotrophs

echo "Complete pred_bw: $(date). Success."
