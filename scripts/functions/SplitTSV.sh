...
SplitTSV_trajectory() {
        ...
        do
                ...
                # Filter barcode file for cells only assigned a trajectory time
                barcodes_filtered="temp_${sample}_metadata.csv"
                head -n 1 "$barcodes" > "$barcodes_filtered"
                awk -F',' -v smp="$sample" '$2 == smp && $4 != ""' "$barcodes" >> "$barcodes_filtered"
                grep ",${sample}," "$barcodes" >> "$barcodes_filtered"
...
# Function:
#SplitTSV_trajectory mouse_young/ mouse_young_split_trajectory/ cell_metadata_young_short_reformatted.csv
