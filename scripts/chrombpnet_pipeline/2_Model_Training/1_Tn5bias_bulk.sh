#!/bin/bash -l
#SBATCH --output=/scratch/prj/stem_cells_pituitary/Georgia/scripts/0_outputs/1_Tn5bias_exp22.out
#SBATCH --job-name=Tn5b_exp22
#SBATCH --gres=gpu
...
chrombpnet bias pipeline \
        -ibam "$experimentDir/TaT1.mRp.clN.sorted.bam" \
        -d "ATAC" \
        -g "$genomeDir/${genome}.fa" \
        -c "$genomeDir/${genome}.chrom.sizes" \
        -p "$peaksDir/TaT1_peaks_no_blacklist_2.bed" \
        -n "$peaksDir/TaT1_output_negatives.bed" \
        -fl "$chromDir/splits/$genome/fold_0.json" \
        -b 0.9 \
        -o "$chromDir/Bias_Models/Tn5_bias_model_experiment_${exp}_bias0.9" \
