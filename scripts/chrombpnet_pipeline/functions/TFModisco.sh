...
TFModisco() {
	...
	do
		...
		final_file="$modiscoDir/${condition}_${n_seqlets}_modisco_results.h5"
		...
		final_file="$modiscoDir/${condition}_${n_seqlets}_modisco_report/motifs.html"
		if [[ -f "$final_file" ]]; then
                        echo "Skipping report for $cell_type: $final_file exists."
                        continue
                else
                        echo "Report for $cell_type does not exist. New run starting."
			rm -rf "$modiscoDir/${condition}_${n_seqlets}_modisco_report"
			echo "Beginning TFmodisco report command"

			$MODISCO_EXE report \
			        -i "$modiscoDir/${condition}_${n_seqlets}_modisco_results.h5" \
			        -o "$modiscoDir/${condition}_${n_seqlets}_modisco_report/" \
			        -s "$modiscoDir/${condition}_${n_seqlets}_modisco_report/" \
			        -m "$jaspar_file"
		fi
